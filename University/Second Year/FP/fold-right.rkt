...
(define (fold-right op null-value xs)
  (if (null? xs) null-value
      ;Същото като при fold-left, но с разменени места на изразите. Това е от значение за операции като делене и изваждане
...
(define fold-right-tests
  (test-suite "Fold-right tests"
    (check-equal? (fold-right / 1 '(1 2 3 4 5)) 15/8) ;(/ (/ (/ (/ (/ 1 1) 2) 3) 4) 5)
